/* File: Hero1.js 
 *
 * Creates and initializes the Hero1 (Dye)
...
function Hero1(atX, atY,keyBoard) {
    ...
    this.mBullets = new BulletSet(BulletTextureArcher);// 赋予英雄子弹
    ...
    this.mHero = new SpriteAnimateRenderable("assets/hero1_animation.png");
...
Hero1.prototype.update = function(enemy,box,aCamera,platformSet) {
    ...
    if(this.effectStartFlag===true){
        ...
        if(this.effectTime>=this.MaxEffectTime){
            ...
            this.mBullets.BulletTexture=BulletTextureArcher;
            this.mShotSpeed=3;
    ...
    if(this.keyBoard===2){
        if (gEngine.Input.isKeyPressed(gEngine.Input.keys.Left)&&this.shotTick>this.mCanShot) {
        gEngine.AudioClips.playACue(shotSound);
        this.mBullets.newAt(this.getXform().getPosition(),this.mfaceDirection);
        this.shotTick=0;
    }
    ...
    if(this.gift===true){
        this.giftTick++;
        if(this.giftTick>=300){//如果时间大于300tick，则不能继续放，并将冷却时间设为0
            this.gift=false;
            this.coldTick=0;
            this.giftTick=0;
        }
         this.mTraceBulletSet.update(this, enemy);
    
    }
    this.mTraceBulletSet.remove(enemy);
...
//统计被击中次数
Hero1.prototype.hitOnce = function() {
    this.mGetHit++;
};

Hero1.prototype.subHP = function(damage) {
    this.HP-=damage;
};
//返回战术统计
Hero1.prototype.getStatus = function(){
    if(this.canUseGift){
...
//移动函数
Hero1.prototype._moveByKeys = function() {
    if(this.keyBoard===1){
    var v = this.getPhysicsComponent().getVelocity();
    ...
    {    
        ...
        gEngine.AudioClips.playACue(ArrowSound);
    ...
    //英雄技能
    if (gEngine.Input.isKeyPressed(gEngine.Input.keys.O)&&this.canUseGift)
    {    
        gEngine.AudioClips.playACue(ArrowSound);
        this.gift=true;
        this.canUseGift=false;
