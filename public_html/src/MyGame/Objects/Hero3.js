...
Hero3.prototype.update = function(enemy,box,aCamera,platformSet) {
    ...
    if(this.keyBoard===2){
        if ((gEngine.Input.isKeyPressed(gEngine.Input.keys.Left)||this.ShotFire)&&this.shotTick>this.mCanShot) {
        gEngine.AudioClips.playACue(shotSound);
        this.mBullets.newAt(this.getXform().getPosition(),this.mfaceDirection);
        this.shotTick=0;
    }
    ...
    if(this.gift===true){
        this.giftTick++;
        if(this.giftTick>=350){//如果时间大于300tick，则不能继续放，并将冷却时间设为0
            this.gift=false;
            this.coldTick=0;
            this.giftTick=0;
            this.mShotSpeed=3;
